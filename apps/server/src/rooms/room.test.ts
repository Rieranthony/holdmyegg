...
const connectMember = (room: Room, index: number) => {
  ...
  const bootstrap = room.connect(join.ticket, userId, socket);
...
describe("Room", () => {
  ...
  it("rate limits waiting-room chat and reports the error back to the sender", () => {
    const room = createRoom();
    const { socket } = connectMember(room, 1);

    for (let index = 0; index < 5; index += 1) {
      room.receiveControl("user-1", {
        type: "chat_send",
        text: `message ${index}`
      });
    }
  ...
  it("clears transient runtime input flags after each live tick", () => {
    ...
    room.receiveRuntimeInput("user-1", encodedInput);
    advanceTime(60);
    room.tick(1 / 60);
  ...
  it("merges queued runtime edges into the newest packet before the next live tick", () => {
    ...
    room.receiveRuntimeInput("user-1", firstPacket);
    room.receiveRuntimeInput("user-1", secondPacket);
